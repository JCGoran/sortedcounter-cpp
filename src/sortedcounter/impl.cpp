...
public:
  ...
  template <typename Container> explicit SortedCounter(const Container &items) {
    m_size = 0;
  ...
  const std::map<T, size_type> &data() const { return m_container; }
